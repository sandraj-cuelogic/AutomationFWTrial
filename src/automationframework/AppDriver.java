...
public class AppDriver {
		...
		if (OS.equalsIgnoreCase("linux")) {
			System.setProperty("phantomjs.binary.path",
					System.getProperty("user.dir") + File.separator + "src"
							+ File.separator + "test" + File.separator + "java"
							+ File.separator + "libs" + File.separator
							+ "phantomjs" + File.separator + "phantomjslinux64");
		}
	}
	...
	private static Capabilities getPhantomCapabilities(String OS) {
		 ...
		 * capabilities.setCapability(
		 * PhantomJSDriverService.PHANTOMJS_GHOSTDRIVER_CLI_ARGS, new String[] {
		 * "--logLevel=2" });
		 */
		return capabilities;
	}
	...
	public static WebDriver getDriver(String browserToUse) {
		...
		String ExcelTest = xl.getXLcellValue("Controller", 1, 0, Configuration.getExcelFile());
		...
		if (browserType.equalsIgnoreCase(FIREFOX)) {
			...
			try {
				driver = new RemoteWebDriver(new URL("http://"
						+ Configuration.RemoteURLIPAndPort() + "/wd/hub"), cap);
			...
			try {
				...
				| !browserType.equalsIgnoreCase(RemoteChrome)
				| !browserType.equalsIgnoreCase(RemoteFirefox)) {
			AutomationLog
					.error("Invalid browser name. Please check your browser Input");
	...
	public static void killChromePhantomInstance(WebDriver driver) {
		...
		try {
			/*
			 * if(Page.driver instanceof PhantomJSDriver &
